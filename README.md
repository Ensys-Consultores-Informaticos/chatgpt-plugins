...
Este directorio es una raíz de marketplace autocontenida para distribuir Gesia MCP 1.10.1. Incluye el servidor MCP local para Windows y los manifiestos que permiten descubrirlo e instalarlo desde ChatGPT Desktop y Codex.
...
plugins/gesia-mcp/server/gesia_mcp-1.10.1.exe
```
...
- Si el servidor no arranca, comprueba que Windows no haya bloqueado `plugins/gesia-mcp/server/gesia_mcp-1.10.1.exe` y verifica su SHA-256 con `Get-FileHash`.
...
`gesia_mcp-1.10.1.exe` es software propietario. Antes de hacer público el repositorio, confirma expresamente que existe autorización para redistribuir el ejecutable. El hash esperado del binario incluido figura en `plugins/gesia-mcp/SHA256SUMS.txt`.
