...
Los scripts están en `skills/fsp-mum/scripts/`, dentro de la raíz del plugin instalado.
...
```bash
python "<PLUGIN>\skills\fsp-mum\scripts\preparar_documentos.py" --carpeta "<la carpeta>" --trabajo "<TRABAJO>"
```
...
```bash
python "<PLUGIN>\skills\fsp-mum\scripts\preparar_documentos.py" --trabajo "<TRABAJO>" --lotes 10
```
...
```bash
python "<PLUGIN>\skills\fsp-mum\scripts\preparar_documentos.py" --trabajo "<TRABAJO>" --fusionar
```
...
```bash
python "<PLUGIN>\skills\fsp-mum\scripts\preparar_documentos.py" --trabajo "<TRABAJO>" --ampliar "<fichero>"
```
...
```bash
python "<PLUGIN>\skills\fsp-mum\scripts\preparar_documentos.py" --trabajo "<TRABAJO>" --estado
```
...
python "<PLUGIN>\skills\fsp-mum\scripts\ejecutar_mum.py" \
    --muestra "<TRABAJO>\muestra.json" --parametros "<TRABAJO>\parametros.json" \
    --facturas "<TRABAJO>\facturas.json" --evaluacion "<TRABAJO>\evaluacion.json" \
    --manifiesto "<TRABAJO>\manifiesto.json" \
    # (si las imágenes las hizo el MCP, el manifiesto está en <TRABAJO>/facturas/: el script lo encuentra igual)
...
```bash
python "<PLUGIN>\skills\fsp-mum\scripts\probar_mum.py"
```
