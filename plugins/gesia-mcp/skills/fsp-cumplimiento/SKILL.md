...
Los scripts están en `skills/fsp-cumplimiento/scripts/`, dentro de la raíz del plugin instalado.
...
```
exportar_consulta(entidad = "muestra", id = <MuestraId>, ruta = "<TRABAJO>/muestra.json")
obtener_entidad('parametros', id = <MuestraId>)      → guárdalo tal cual en <TRABAJO>/parametros.json
```

Y si el auditor ya evaluó la prueba (la `_AN` existe), también:

```
exportar_consulta(entidad = "evaluacion", id = <MuestraId>, ruta = "<TRABAJO>/evaluacion.json")
```
...
Los ficheros exportados van a `<TRABAJO>`, que es una carpeta local: aquí no hay contenedor ni copias que subir. Reutiliza la misma carpeta en toda la sesión.
...
espera su confirmación o sus cambios. Guárdalo en `<TRABAJO>/roles.json`:
...
`preparar_documentos.py` lee tal cual: **sube esa carpeta dentro del mismo `<TRABAJO>` del paso 2**.
**No es otro `<TRABAJO>`**: `muestra.json`, `parametros.json` y `facturas/` conviven en el mismo
...
```bash
python "<PLUGIN>\skills\fsp-cumplimiento\scripts\preparar_documentos.py" --carpeta "<la carpeta>" --trabajo "<TRABAJO>"
```
...
```bash
python "<PLUGIN>\skills\fsp-cumplimiento\scripts\preparar_documentos.py" --trabajo "<TRABAJO>" --lotes 10
```
...
```bash
python "<PLUGIN>\skills\fsp-cumplimiento\scripts\preparar_documentos.py" --trabajo "<TRABAJO>" --fusionar
```
...
```bash
python "<PLUGIN>\skills\fsp-cumplimiento\scripts\preparar_documentos.py" --trabajo "<TRABAJO>" --ampliar "<fichero>"
...
De cada factura anota lo siguiente, y **escribe `<TRABAJO>/facturas.json` cada cinco o seis
...
```bash
python "<PLUGIN>\skills\fsp-cumplimiento\scripts\preparar_documentos.py" --trabajo "<TRABAJO>" --estado
```
...
python "<PLUGIN>\skills\fsp-cumplimiento\scripts\ejecutar_fsp.py" \
    --muestra "<TRABAJO>\muestra.json" --parametros "<TRABAJO>\parametros.json" \
    --facturas "<TRABAJO>\facturas.json" --roles "<TRABAJO>\roles.json" \
    --evaluacion "<TRABAJO>\evaluacion.json" \
    --manifiesto "<TRABAJO>\manifiesto.json" \
...
**Aquí el papel es uno solo**: el del expediente. No hay copia de chat ni contenedor por el que viajar, así que en cuanto lo desanonimizas lleva los nombres reales en el disco del auditor. Dilo tal cual, para que sepa que ese fichero ya no es anónimo y trate el sitio donde lo deje en consecuencia. **El diccionario de nombres no se borra**: vive con el encargo,
...
auditor con la ruta, y en este canal bórralas tú, que compartes disco con el auditor). `facturas.json` y `roles.json`
...
```bash
python "<PLUGIN>\skills\fsp-cumplimiento\scripts\probar_fsp.py"
```
