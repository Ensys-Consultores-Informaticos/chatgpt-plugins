...
Los scripts están en `skills/identificacion-riesgos/scripts/`, dentro de la raíz
del plugin instalado.

**Sustituye los marcadores de los ejemplos por rutas reales antes de ejecutar.** No
uses variables de shell: en Windows el intérprete no siempre es el mismo y una
...
**El MCP y los scripts comparten disco**, así que los volcados de
`exportar_consulta` se leen donde el MCP los dejó: no hay que subirlos ni copiarlos.
Exporta a `<TRABAJO>` y pásale esa misma ruta a los scripts.
...
exportar_consulta(entidad = "catalogo_riesgos", ruta = "<TRABAJO>/catalogo.json")
```
...
```bash
python "<PLUGIN>/skills/identificacion-riesgos/scripts/extraer_catalogo.py" \
    --catalogo "<TRABAJO>/catalogo.json" --salida "<TRABAJO>" \
...
```
exportar_consulta(entidad = "cifras_balance", ruta = "<TRABAJO>/balance.json")
exportar_consulta(entidad = "cifras_pyg",     ruta = "<TRABAJO>/pyg.json")
exportar_consulta(entidad = "cifras_ratios",  ruta = "<TRABAJO>/ratios.json")
```
...
```bash
python "<PLUGIN>/skills/identificacion-riesgos/scripts/analizar_cifras.py" \
    --balance "<TRABAJO>/balance.json" --pyg "<TRABAJO>/pyg.json" \
    --ir-t <IR_T> --ir-t-origen gesia \
    --ratios "<TRABAJO>/ratios.json" \
...
```bash
python "<PLUGIN>/skills/identificacion-riesgos/scripts/analizar_ra.py" \
    --ra "<TRABAJO>/ratios.json" --ejercicios "2025,2024,2023,2022,2021" \
    --ir-t 200000 \
...
```bash
python "<PLUGIN>/skills/identificacion-riesgos/scripts/mostrar_riesgos.py" --catalogo "<TRABAJO>/riesgos.json"     --ids 18,27,33,80
```
...
```bash
python "<PLUGIN>/skills/identificacion-riesgos/scripts/validar_seleccion.py" \
...
```bash
python "<PLUGIN>/skills/identificacion-riesgos/scripts/generar_informe.py" \
...
**El destino es la carpeta del expediente**, directamente:
`<expediente>/InformesGesia/IdentificacionRiesgos/`. El script crea el árbol si no
existe.

**Si el expediente está en OneDrive, el entorno puede rechazar la escritura antes de
ejecutar nada** y pedir autorización expresa para escribir datos contables ahí. No es
un fallo: pídesela al usuario explicando que es el informe que ha encargado, y repite
la misma orden.
...
`limpiar_exportaciones()`, y esa es la vía: no hay que decirle qué ficheros, porque
borra los que él mismo escribió. Si alguno no se deja borrar —abierto en otra
aplicación, típicamente—, lo dice con su ruta: trasládala al usuario.

El directorio de trabajo lo borras tú: elimina `<TRABAJO>` con la orden que
corresponda a la consola en la que estés.

Los volcados van a la carpeta temporal del sistema o a `<TRABAJO>`, y **nunca a
`InformesGesia`**, donde solo debe quedar el entregable.

Si el borrado falla, **di las rutas exactas** de lo que queda. Sin adornos: son
datos de su cliente y tiene que poder borrarlos él. Lo que no vale es dejarlo en un
«puede que queden temporales» sin decir dónde.
...
```bash
python "<PLUGIN>/skills/identificacion-riesgos/scripts/probar_riesgos.py"
python "<PLUGIN>/skills/identificacion-riesgos/scripts/probar_riesgos.py" --catalogo "<TRABAJO>/riesgos.json"
```
