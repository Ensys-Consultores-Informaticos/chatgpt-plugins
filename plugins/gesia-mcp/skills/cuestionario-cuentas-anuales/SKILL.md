...
Los scripts están en `skills/cuestionario-cuentas-anuales/scripts/`, dentro de la
raíz del plugin instalado.
...
**El dato entra por el MCP, siempre.** Los scripts no hablan con el API de Gesia. El
cuestionario entra en una sola llamada a `exportar_consulta`, que lo deja en disco: son
154 filas y ~94.000 caracteres —`consultar_gesia` cortaría en 60.000—, y tú lees de ese
fichero solo la sección que estás contestando.

**El MCP y los scripts comparten disco**, así que el fichero se lee donde el MCP lo
dejó: no hay que subirlo ni copiarlo a ninguna parte. Exporta a `<TRABAJO>` y pásale esa
misma ruta a los scripts.
...
exportar_consulta(
  ...
  ruta = "<TRABAJO>/cuestionario.json")
```
...
```bash
python "<PLUGIN>/skills/cuestionario-cuentas-anuales/scripts/verificar_contrato.py" \
    --cuestionario "<TRABAJO>/cuestionario.json" --guia "<el código que hayas localizado>"
```
...
```bash
python "<PLUGIN>/skills/cuestionario-cuentas-anuales/scripts/mostrar_seccion.py" --cuestionario "<TRABAJO>/cuestionario.json" --punto 3
```
...
```bash
python "<PLUGIN>/skills/cuestionario-cuentas-anuales/scripts/generar_xlsx.py" \
    --cuestionario "<TRABAJO>/cuestionario.json" \
...
`<expediente>/InformesGesia/CuestionarioCuentasAnuales/`. El script crea el árbol si no
existe. Vale igual para el papel de trabajo del paso siguiente.

**Si el expediente está en OneDrive, el entorno puede rechazar la escritura antes de
ejecutar nada** y pedir autorización expresa para escribir datos contables ahí. No es un
fallo: pídesela al usuario explicando que es el papel que ha encargado, y repite la
misma orden.
...
```bash
python "<PLUGIN>/skills/cuestionario-cuentas-anuales/scripts/generar_papel.py" \
    --cuestionario "<TRABAJO>/cuestionario.json" \
    --respuestas "<TRABAJO>/respuestas.json" \
    --generado "<AAAA-MM-DD>" \
...
`limpiar_exportaciones()`, y esa es la vía: borra lo que él mismo escribió sin que
haya que decirle cuál. Si alguno no se deja —un fichero abierto en otra aplicación,
típicamente—, lo dice con su ruta: trasládala al usuario.
...
`InformesGesia`**, donde solo debe quedar el entregable. Si el borrado falla, di las
rutas exactas de lo que queda: son datos del cliente.
